...
#
#  Por si acaso borramos....
#
OUTLIST="$CPUMEMFILE $DEVFILE"
for file in $OUTLIST
do 
	if [ -f $file ]
	then
...
case "$3" in
		...
		then
			...
			/usr/bin/nawk -v "CAD=$CHAIN1" -f $BASEDIR/bin/strip-data.awk tmp2-cpu.csv > ${CSVCPUMEMOUTHIST}
			rm tmp2-cpu.csv
		else 
		...
		for ana in $ANALISYS
	        do
        	        ...
        	        ANA_OUTFILE=diskuse_$ana-hist-$MAQ.csv
               		export LC_ALL=es_ES
			/usr/bin/nawk -f $BASEDIR/bin/$ana-devs.awk ${CSVDEVOUTHIST}  > $ANA_OUTFILE
			export LC_ALL=C
       		done
		$BASEDIR/bin/buildservhtml.sh $MAQ $3
                ;;
	mensual)
                INST_TIME=`/usr/bin/date +'%b %d %H:%M:%S'`
		...
		then
                	...
                	PAGESIZE=`grep $MAQ $PAGEMEMFILE | awk '{print $2}'`
                	/usr/bin/nawk -v "PAGE=$PAGESIZE" -v "MEMS=$MEMSIZE" -f $BASEDIR/bin/percentMEMFree.awk tmp-cpu.csv > tmp2-cpu.csv
                        /usr/bin/nawk -v "CAD=$CHAIN1" -f $BASEDIR/bin/strip-data.awk tmp2-cpu.csv > $CSVCPUMEMOUT
                        rm tmp2-cpu.csv
                else
                        /usr/bin/nawk -v "CAD=$CHAIN1" -f $BASEDIR/bin/strip-data.awk tmp-cpu.csv > $CSVCPUMEMOUT
                fi
                #
                # CSV de discos
                #
		$GRAPHEXEC -outputCSV tmp-dev.csv -input $DEVFILE 2>> $ERRORLOG
		/usr/bin/nawk -v "CAD=$CHAIN1" -f $BASEDIR/bin/strip-data.awk tmp-dev.csv > $CSVDEVOUT
		rm tmp-cpu.csv
		rm tmp-dev.csv
		#
                # Con el CSV de los discos generamos otros tres con el %busy, blocks/s y avservt
                ...
                do
                        ...
                        export LC_ALL=es_ES
			/usr/bin/nawk -f $BASEDIR/bin/$ana-devs.awk ${CSVDEVOUT}  > $ANA_OUTFILE
			export LC_ALL=C
                done
		rm $CSVDEVOUT
		rm $DEVFILE
