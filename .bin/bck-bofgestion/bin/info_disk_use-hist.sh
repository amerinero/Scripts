...
echo "$INST_TIME - Generando CSV's con datos del uso de discos...desde historico" >> $LOGFILE
for serv in $LISTASERVS
do
	INFILE=$serv/dev-hist-$serv.csv
	export LC_ALL=es_ES
	for ana in $ANALISYS
	do
		INST_TIME=`/usr/bin/date +'%b %d %H:%M:%S'`
		echo "$INST_TIME - Generando $ana para $serv ...desde historico" >> $LOGFILE
		OUTFILE=$serv/diskuse-hist_$ana-$serv.csv
		nawk -f $BASEDIR/bin/$ana-devs.awk $INFILE > $OUTFILE
	done
done
INST_TIME=`/usr/bin/date +'%b %d %H:%M:%S'`
echo "$INST_TIME - CSV's con datos de uso de disco desde historico generados." >> $LOGFILE
