...
# a partir del sar -A de una máquina AIX.
# 
#
...
# PROMPT> LC_ALL=C; export LC_ALL; sar -A | awk -f sarAIX2DB.awk |  sqlite3 test.db
...
#	printf "MARK=%d,COLS[2]=%s\n",MARK,COLS[2];
	...
	if (COLS[2] ~ /ksched/ ) # sar -k
        {

                for (cnt=2; cnt <= NF; cnt++)
                        printf "insert into %s values('20%s-%s-%s %s','%s','KERNEL','%s',%s);\n"\
                        ,TABLE,FECHA[3],FECHA[1],FECHA[2],$1,SYSNAME,COLS[cnt],$cnt;
        }

	if (COLS[2] ~ /cswch/) # para sar -w
        {
